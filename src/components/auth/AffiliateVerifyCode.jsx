...
function AffiliateVerifyCode() {
  const [isLoading, setIsLoading] = useState(false);
  const [verificationCode, setVerificationCode] = useState(Array(6).fill(""));
  const [timer, setTimer] = useState(60); // Initial 1 minute timer for resend
  const [codeSentTime, setCodeSentTime] = useState(null);
  const [attempts, setAttempts] = useState(0); // Track number of resend attempts
  ...
  useEffect(() => {
    ...
    if (codeSentTime) {
      const countdown = setInterval(() => {
        ...
        const remainingTime = Math.max(0, timer - elapsedTime); // Calculate remaining time
  ...
  const handleResendCode = () => {
    let newTimer = 60; // Default to 1 minute for first resend

    if (attempts === 2) {
      newTimer += 60; // Add 1 minute after second attempt
    } else if (attempts >= 3) {
      newTimer = 600; // Set to 10 minutes after third attempt
    }
...
export default AffiliateVerifyCode;